...
import { INDIA_STATES, LOCATION_DATA_URL, FALLBACK_DISTRICTS, MARKET_DIRECTORY } from './data/locationData.js';
...
class AgriPredictApp {
  constructor() {
    this.currentRoute = '/';
    this.currentLang = (() => { try { return localStorage.getItem('agripredict_language') || 'en'; } catch (e) { return 'en'; } })();
  ...
  async init() {
    ...
    // 2. Load India-wide location data for cascading selectors
    await this.loadLocationData();

    // 3. Setup event listeners and router
    this.setupGlobalEvents();
    this.setupRouter();

    // 4. Determine initial route from URL
  ...
  setupGlobalEvents() {
    ...
    if (langSelect) {
      ...
      langSelect.addEventListener('change', (e) => {
        ...
        try { localStorage.setItem('agripredict_language', this.currentLang); } catch (e) {}
  ...
  async loadLocationData() {
    try {
      const response = await fetch(LOCATION_DATA_URL, { signal: AbortSignal.timeout(5000) });
      if (response.ok) {
        const payload = await response.json();
        const rows = Array.isArray(payload) ? payload : (payload.states || []);
        this.locationData = rows.map(row => ({
          state: row.state,
          districts: Array.isArray(row.districts) ? row.districts : []
        }));
        this.locationDataLoaded = this.locationData.length > 0;
      }
    } catch (e) {
      console.warn('India district dataset unavailable; using local location fallbacks:', e.message);
    }

    // Always keep the full State/UT selector available even if the remote dataset is offline.
    const byState = new Map(this.locationData.map(x => [x.state, x.districts]));
    INDIA_STATES.forEach(state => {
      if (!byState.has(state)) {
        this.locationData.push({ state, districts: FALLBACK_DISTRICTS[state] || [] });
      } else if ((!byState.get(state) || byState.get(state).length === 0) && FALLBACK_DISTRICTS[state]) {
        byState.set(state, FALLBACK_DISTRICTS[state]);
      }
    });
  }
  ...
  getDistrictsForState(state) {
    ...
    const row = this.locationData.find(x => String(x.state).trim().toLowerCase() === normalized)
      || this.locationData.find(x => String(x.state).trim().toLowerCase().includes(normalized) || normalized.includes(String(x.state).trim().toLowerCase()));
    const fallback = FALLBACK_DISTRICTS[state] || [];
    return [...new Set([...(row?.districts || []), ...fallback])].filter(Boolean).sort((a,b) => a.localeCompare(b));
  }

  async refreshMarketOptions() {
    const state = document.getElementById('inputState')?.value || '';
    const district = document.getElementById('inputDistrict')?.value || '';
    const commodity = document.getElementById('commoditySelect')?.value || this.formData.commodityId || '';
    const select = document.getElementById('inputMarket');
    if (!select) return;
    ...
    try {
      const result = await ApiService.getLiveMarketData({ state, district, commodity, limit: 1000 });
      (result?.records || []).forEach(record => {
        const market = record.Market || record.market || record.market_name || record.mandi || record.APMC;
        if (!market) return;
        const name = String(market).trim();
        const recordState = String(record.State || record.state || '').trim().toLowerCase();
        const recordDistrict = String(record.District || record.district || '').trim().toLowerCase();
        const recordCommodity = String(record.Commodity || record.commodity || '').trim().toLowerCase();
        const stateMatch = !state || !recordState || recordState === state.toLowerCase();
        const districtMatch = !district || !recordDistrict || recordDistrict === district.toLowerCase();
        const commodityMatch = !commodity || !recordCommodity || recordCommodity.includes(String(commodity).toLowerCase());
        if (stateMatch && districtMatch && commodityMatch) exactNames.add(name);
        if (stateMatch && commodityMatch) stateNames.add(name);
        allNames.add(name);
      });
    } catch (e) {
      console.warn('Market option lookup failed:', e.message);
    }

    // Local dropdown-only directory: used only when the live/cached source does not provide enough options.
    MARKET_DIRECTORY.forEach(market => {
      const stateMatch = !state || String(market.state).toLowerCase() === String(state).toLowerCase();
      const districtMatch = !district || String(market.district).toLowerCase() === String(district).toLowerCase();
      const commodityMatch = !commodity || (market.commodities || []).length === 0 || market.commodities.includes(commodity);
      if (stateMatch && districtMatch && commodityMatch) exactNames.add(market.name);
      if (stateMatch && commodityMatch) stateNames.add(market.name);
      allNames.add(market.name);
    });

    // Keep existing local mandi records as a safe fallback. These records are used only for the selector.
    MANDIS_DATABASE.forEach(mandi => {
      const stateMatch = !state || String(mandi.state).toLowerCase() === String(state).toLowerCase();
      const districtMatch = !district || String(mandi.district).toLowerCase() === String(district).toLowerCase();
      const commodityMatch = !commodity || !Array.isArray(mandi.bestFor) || mandi.bestFor.includes(commodity);
      if (stateMatch && districtMatch && commodityMatch) exactNames.add(mandi.name);
      if (stateMatch && commodityMatch) stateNames.add(mandi.name);
      allNames.add(mandi.name);
    });

    // Prefer exact State + District + Commodity matches. If fewer than 11 exist, fill from the selected State + Commodity.
    // This keeps the selector useful even when the government API/cached dataset has fewer records.
    const combined = new Set(exactNames);
    for (const name of stateNames) {
      if (combined.size >= 11) break;
      combined.add(name);
    }
    ...
    const preferred = this.formData.preferredMarket || '';
    const options = [...combined].filter(Boolean).sort((a,b) => a.localeCompare(b));
    if (preferred && !options.includes(preferred)) options.unshift(preferred);
    if (!options.length) options.push('Other / Nearby Market');

    select.innerHTML = '<option value="">Select preferred mandi / market</option>' +
      options.map(name => `<option value="${this.escapeHtml(name)}" ${name === preferred ? 'selected' : ''}>${this.escapeHtml(name)}</option>`).join('');
    select.disabled = false;
    this.marketOptions = options;
    this.applyLanguageToDOM();
  }

  refreshDistrictOptions(preserveValue = true) {
    const stateSelect = document.getElementById('inputState');
    const districtSelect = document.getElementById('inputDistrict');
    if (!stateSelect || !districtSelect) return;
    const previous = preserveValue ? (this.formData.district || '') : '';
    const districts = this.getDistrictsForState(stateSelect.value);
    districtSelect.innerHTML = '<option value="">Select district</option>' +
      districts.map(d => `<option value="${this.escapeHtml(d)}" ${d === previous ? 'selected' : ''}>${this.escapeHtml(d)}</option>`).join('');
    if (previous && districts.includes(previous)) districtSelect.value = previous;
    else if (!districtSelect.value && districts.length === 1) districtSelect.value = districts[0];
    this.applyLanguageToDOM();
  }
  ...
  applyLanguageToDOM() {
    ...
    nodes.forEach(node => {
      if (!Object.prototype.hasOwnProperty.call(node, '__agriBaseText')) {
        Object.defineProperty(node, '__agriBaseText', { value: node.nodeValue, writable: true });
      }
      node.nodeValue = lang === 'en' ? node.__agriBaseText : replaceText(node.__agriBaseText);
    });
    document.querySelectorAll('input[placeholder], textarea[placeholder]').forEach(el => {
      if (el.dataset.agriBasePlaceholder === undefined) el.dataset.agriBasePlaceholder = el.getAttribute('placeholder') || '';
      const base = el.dataset.agriBasePlaceholder;
      el.setAttribute('placeholder', lang === 'en' ? base : replaceText(base));
    });
    document.querySelectorAll('[title]').forEach(el => {
      if (el.dataset.agriBaseTitle === undefined) el.dataset.agriBaseTitle = el.getAttribute('title') || '';
      const base = el.dataset.agriBaseTitle;
      el.setAttribute('title', lang === 'en' ? base : replaceText(base));
    });
  }
  ...
  navigateTo(route, updateHistory = true) {
    ...
    // Apply the selected language to the freshly rendered route without changing business logic.
    requestAnimationFrame(() => this.applyLanguageToDOM());
  }
  ...
  renderInputView(container) {
          ...
          <form id="produceAnalysisForm" class="space-y-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div class="relative">
                  <select id="inputState" class="w-full pl-10 pr-4 py-3.5 rounded-2xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white font-bold text-base focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                    ${INDIA_STATES.map(state => `<option value="${this.escapeHtml(state)}" ${state === (fd.state || 'Andhra Pradesh') ? 'selected' : ''}>${this.escapeHtml(state)}</option>`).join('')}
                  </select>
              ...
              <div>
                ...
                <div class="relative">
                  <select id="inputDistrict" class="w-full pl-10 pr-4 py-3.5 rounded-2xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white font-bold text-base focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                    <option value="">Select district</option>
                    ${this.getDistrictsForState(fd.state || 'Andhra Pradesh').map(d => `<option value="${this.escapeHtml(d)}" ${d === (fd.district || 'Kurnool') ? 'selected' : ''}>${this.escapeHtml(d)}</option>`).join('')}
                  </select>
                  <span class="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div class="relative">
                  <select id="inputMarket" class="w-full pl-10 pr-4 py-3.5 rounded-2xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white font-bold text-base focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                    <option value="">Select preferred mandi / market</option>
                  </select>
                  <span class="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none">
    ...
    // Cascading State -> District -> Preferred Mandi selectors
    const stateSelect = document.getElementById('inputState');
    const districtSelect = document.getElementById('inputDistrict');
    if (stateSelect) {
      stateSelect.addEventListener('change', async () => {
        this.formData.state = stateSelect.value;
        this.refreshDistrictOptions(false);
        this.formData.district = document.getElementById('inputDistrict')?.value || '';
        await this.refreshMarketOptions();
      });
    }
    if (districtSelect) {
      districtSelect.addEventListener('change', async () => {
        this.formData.district = districtSelect.value;
        await this.refreshMarketOptions();
      });
    }
    const commoditySelector = document.getElementById('commoditySelect');
    if (commoditySelector) {
      commoditySelector.addEventListener('change', () => this.refreshMarketOptions());
    }
    this.refreshMarketOptions();
      ...
      <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 max-w-xl w-full border border-slate-200 dark:border-slate-700 shadow-2xl relative text-left">
        ...
        <div class="my-5 space-y-3 text-xs sm:text-sm text-slate-700 dark:text-slate-300">
          <div class="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 space-y-1.5">
            ...
            <div class="flex items-center gap-2 pt-1">• <span>Contact Number:</span> <a href="tel:${buyer.phone || ''}" class="font-black text-emerald-600 dark:text-emerald-400 hover:underline">${buyer.phone || 'Not available'}</a></div>
